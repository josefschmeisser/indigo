...
"""
import tensorflow as tf
q = tf.FIFOQueue(capacity=1, [tf.int16], shared_name="dagger_sync")
...
def mainloop():
    dagger_sync_q = tf.FIFOQueue(1, [tf.int16], shared_name="dagger_sync_q")
    dagger_sync = tf.placeholder(tf.int16)
    while True:
        #
        pass
        if request == RESET_ENV:
            sess.run(enqueue, feed_dict={dagger_sync: Status.DONE})
